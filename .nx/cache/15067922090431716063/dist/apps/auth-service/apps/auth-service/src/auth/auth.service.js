...
let AuthApiService = AuthApiService_1 = class AuthApiService {
    userModel;
    authService;
    paymentClientService;
    logger = new common_1.Logger(AuthApiService_1.name);
    ...
    async login(dto) {
        return this.loginByRole(dto, utils_1.Role.USER, () => new Errors.UserNotExist());
    }
    async loginAdmin(dto) {
        return this.loginByRole(dto, utils_1.Role.ADMIN, () => new Errors.AdminNotExist());
    }
    async loginVendor(dto) {
        return this.loginByRole(dto, utils_1.Role.VENDOR, () => new Errors.VendorNotExist());
    }
    async updateUserRole(userId, role) {
        const existing = await this.userModel.findOne({ where: { id: userId, is_deleted: false } });
        if (!existing) {
            throw new Errors.UserNotFound();
        }
        await this.userModel.update({ id: userId }, { role });
        await this.authService.clearUserCache(userId);
        return { data: { message: 'User role updated successfully.', user_id: userId, role } };
    }
    async loginByRole(dto, role, notExistErrorFactory) {
        try {
...
exports.AuthApiService = AuthApiService = AuthApiService_1 = __decorate([
