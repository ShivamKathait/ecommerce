...
let VendorsService = VendorsService_1 = class VendorsService {
    ...
    async register(createVendorDto, user) {
        ...
        const accountId = await this.paymentClientService.createConnectAccount(user.email);
        const saveData = {
            business_name,
            gst_number,
        ...
        const link = await this.paymentClientService.generateOnboardingLink(accountId, vendor.id);
        return { link };
    }
    async refresh_onboarding(vendorId) {
        ...
        const link = await this.paymentClientService.generateOnboardingLink(vendor.stripe_connect_id, vendor.id);
        return { link };
    }
