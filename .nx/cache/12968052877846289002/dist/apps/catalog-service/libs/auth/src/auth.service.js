...
let AuthService = AuthService_1 = class AuthService {
    ...
    constructor(jwtService, configService, userModel, sessionModel, cacheManager) {
        ...
        this.JWT_ACCESS_EXPIRY = this.configService.get('JWT_ACCESS_EXPIRY');
        ...
        this.BCRYPT_SALT_ROUNDS = this.configService.get('BCRYPT_SALT_ROUNDS');
    }
    ...
    async generateTempToken(id, email, role) {
        return this.jwtService.sign({ id, email, role }, { secret: this.JWT_ACCESS_TEMP_SECRET, expiresIn: this.JWT_ACCESS_TEMP_EXPIRY });
    }
    async createSession(user_id, role) {
        return await this.sessionModel.save({ user: { id: user_id }, role: role });
    }
