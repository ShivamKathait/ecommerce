...
let OrdersController = class OrdersController {
    ...
    confirm(orderId, dto, req) {
        const authHeader = req.headers?.authorization ?? '';
        const token = authHeader.startsWith('Bearer ')
            ? authHeader.slice(7)
            : authHeader;
        return this.ordersService.confirmOrder(orderId, req.user.id, token, dto.paymentIntentId);
    }
    cancel(orderId, req) {
        const authHeader = req.headers?.authorization ?? '';
        const token = authHeader.startsWith('Bearer ')
...
__decorate([
    ...
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, confirm_order_dto_1.ConfirmOrderDto, Object]),
    __metadata("design:returntype", void 0)
